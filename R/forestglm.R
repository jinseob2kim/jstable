...
TableSubgroupGLM <- function(formula, var_subgroup = NULL, var_cov = NULL, data, family = "binomial", decimal.estimate = 2, decimal.percent = 1, decimal.pvalue = 3) {
  ...
  if (is.null(var_subgroup)) {
    ...
    CI <- round(matrix(c(cc[2:(1+ncoef), 1] - qnorm(0.975) * cc[2:(1+ncoef), 2], cc[2:(1+ncoef), 1] + qnorm(0.975) * cc[2:(1+ncoef), 2]), ncol = 2,
                       dimnames = list(paste0(xlabel, xlev[[1]][-1]), c("2.5 %", "97.5 %"))), decimal.estimate)
    
    if (family %in% c("binomial", "poisson", "quasipoisson")) {
      Point.Estimate <- round(exp(stats::coef(model)), decimal.estimate)[2:(1+ncoef)]
      CI <- round(exp(matrix(c(cc[2:(1+ncoef), 1] - qnorm(0.975) * cc[2:(1+ncoef), 2], cc[2:(1+ncoef), 1] + qnorm(0.975) * cc[2:(1+ncoef), 2]), ncol = 2,
                             dimnames = list(paste0(xlabel, xlev[[1]][-1]), c("2.5 %", "97.5 %")))), decimal.estimate)
    }
    ...
    if (ncoef < 2) {
      ...
      data.frame(Variable = c("Overall", rep("", length(Point.Estimate))), Count = c(length(model$y), rep("", length(Point.Estimate))), Percent = c(100, rep("", length(Point.Estimate))),
    ...
    if (family %in% c("binomial", "poisson", "quasipoisson")) {
      ...
      CI <- model %>% 
        purrr::map(function(model) {
          cc0 <- tryCatch(summary(model)$coefficients, error = function(e) return(NA))
          
          ci0 <- matrix(NA, ncol = 2, nrow = max(length(xlev[[1]])-1, 1), dimnames = list(paste0(xlabel, xlev[[1]][-1]), c("2.5 %", "97.5 %")))
          for (i in rownames(ci0)) {
            ci0[i, 1] <- tryCatch(cc0[i, 1] - stats::qnorm(0.975) * cc0[i, 2], error = function(e) return(NA))
            ci0[i, 2] <- tryCatch(cc0[i, 1] + stats::qnorm(0.975) * cc0[i, 2], error = function(e) return(NA))
          }
    ...
    pv <- model %>% 
      purrr::map(function(model){
        ...
        for (i in names(pvl)) {
          pvl[i] <- tryCatch(cc0[i, 4], error = function(e) return(NA))
        }
    ...
    if (ncoef < 2) {
      ...
      data.frame(Variable = unlist(lapply(label_val, function(x) c(x, rep("", length(xlev[[1]])-1)))), Count = unlist(lapply(Count, function(x) c(x, rep("", length(xlev[[1]])-1)))), Percent = unlist(lapply(round(Count / sum(Count) * 100, decimal.percent), function(x) c(x, rep("", length(xlev[[1]])-1)))),
