...
cox2.display <- function(cox.obj.withmodel, dec = 2, msm = NULL, pcut.univariate = NULL, data_for_univariate = NULL) {
  ...
  if (length(xf[!grepl(":", xf)]) == 0) {
    xfc <- NULL
  } else {
    xfc <- xf[!grepl(":", xf)]
  }
  categorical_vars <- xfc[sapply(mdata[xfc], is.factor)] 
    ...
    if (!is.null(msm)) {
      ...
      if(is.null(pcut.univariate)){
        ...
        if (length(significant_vars) == 0 ){
          ...
          rownames(mul) <- sapply(rownames(mul), function(x){
            if(grepl(":", x)){
              vars <- unlist(strsplit(x, ":"))
              reversed <- paste(rev(vars), collapse = ":")
              
              if (reversed %in% rownames(uni.res)) reversed else x
            } else x
          })
